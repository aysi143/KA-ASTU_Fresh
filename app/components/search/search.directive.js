		...
		link: function(scope, element, attrs) {
			...
			scope.getSuggestions = function(){
				...
				if (scope.inputValue) {
					...
					switch(scope.type) {
						case 'web':
							scope.suggUrl = 'https://yatab.net/suggestions.php?q=' + encodeURIComponent(scope.inputValue);
							break;

						case 'cities':
							scope.suggUrl = 'https://yatab.net/geoapi/city?q=' + encodeURIComponent(scope.inputValue);
							break;
				...
				if (scope.type == 'cities') {
					...
					if (query != undefined && query.length > 0) {
						...
						switch($rootScope.searchType) {
							case 'web':
								scope.searchUrl = 'https://www.bing.com/search?q=';
								break;
							default:
								scope.searchUrl = 'https://www.bing.com/search?q=';
								break;
						...
						window.top.location = scope.searchUrl + encodeURIComponent(query);
					}
				}
			}
