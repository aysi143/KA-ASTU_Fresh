App.factory('weatherService', function($rootScope, cacheService, $filter, $timeout) {
    ...
    factory.weatherCallback = function(response, responseCache, error) {
        ...
        if (weatherSuccess) {
			...
			$rootScope.currentWeather = {
				...
				'wind_speed': $filter('number')(channel.wind.speed*3.6, 1) + ' kph',
				'wind_speed_mph': $filter('number')(channel.wind.speed*2.2369, 1) + ' mph',
				'wind_direction': channel.wind.deg,
				'humidity': channel.main.humidity,
				'pressure': Math.floor(channel.main.pressure/1.3332239) + ' mm Hg',
				"pressureF": $filter('number')(channel.main.pressure*0.02953, 1) + " inches",
				'temp': $filter('number')(channel.main.temp, 0),
				'tempF': Math.floor(channel.main.temp * 1.8) + 32,
				'icon': channel.weather[0].icon,
				'icon_id': channel.weather[0].id
	...
	factory.getCurrentWeather = function() {
        ...
        cacheService.get(
            'weather',
            'https://pro.openweathermap.org/data/2.5/weather?lat='+ encodeURIComponent(lat) +'&lon='+ encodeURIComponent(lon) +'&mode=json&units=metric&appid='+ WEATHER_APPID,
    ...
    factory.refreshWeather = function() {
        ...
        cacheService.refresh(
            'hourlyForecast',
            'https://pro.openweathermap.org/data/2.5/forecast?lat='+ encodeURIComponent(lat) +'&lon='+ encodeURIComponent(lon) +'&mode=json&units=metric&cnt=8&appid='+ WEATHER_APPID,
            factory.hourlyForecastCallback
        );

        cacheService.refresh(
            'dailyForecast',
            'https://pro.openweathermap.org/data/2.5/forecast/daily?lat='+ encodeURIComponent(lat) +'&lon='+ encodeURIComponent(lon) +'&mode=json&units=metric&cnt=6&appid='+ WEATHER_APPID,
